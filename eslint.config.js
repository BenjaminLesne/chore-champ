...
export default defineConfig(
    ...
    rules: {
      ...
      "no-restricted-syntax": [
        ...
        },
        {
          ...
          message:
            "Use the `cn()` helper from `@/lib/utils` instead of template literals in className.",
        },
        {
          selector:
            'TSAsExpression:not([typeAnnotation.typeName.name="const"])',
          message:
            "Avoid `as` type assertions. Narrow the type upstream (e.g. fix the prop/param type) so the cast is unnecessary. Use type guards, generics, or `satisfies` as alternatives. `as const` is allowed.",
        },
        {
          selector:
            "CallExpression[callee.object.name='t'][callee.property.name='raw']",
          message:
  ...
  // ── Architecture boundaries ──────────────────────────────────────────
  {
    ...
    settings: {
      ...
      "boundaries/elements": [
        ...
        {
          type: "app-editor",
          pattern: ["src/app/**/\\(editor\\)/**"],
          mode: "full",
        },
        {
          type: "app-public",
          pattern: ["src/app/**/\\(public\\)/**"],
          mode: "full",
        },
        {
          type: "app-checkout",
          pattern: ["src/app/**/\\(checkout\\)/**"],
          mode: "full",
        },
        {
  ...
  // ── Allow process.env in env.ts, middleware, and config files ──
  {
    files: [
      "src/env.ts",
      "src/proxy.ts",
  ...
  // ── E2E tests: relax rules that don't apply to Playwright specs ──
  {
    files: ["e2e-tests/**/*.ts"],
    rules: {
